//Sipes, Patricia
//This is primarily where the game logic goes.

#include "MyDirectX.h"
using namespace std;

const string APPTITLE = "Catch-A-Bomb";
...
bool Game_Init( HWND window )
{
   ...
   if( ! bomb_surf ) 
   { 
      MessageBox( window,"Error loading bomb",
                  "Error", 0 );
   return false; 
   } 
   ...
   if( ! bucket_surf ) 
   {
      MessageBox( window, "Error loading bucket",
         "Error", 0 ); 
...
{ 
   ...
   if(bomb.y > SCREENH) 
   { 
      MessageBox( 0," KERPLEWIE!!"," YOU STINK", 0); 
      gameover = true; 
   } 
   ...
   if( XInput_Controller_Found() ) 
   {
      ...
      if (controllers[0].wButtons & XINPUT_GAMEPAD_DPAD_LEFT) bucket.x -= 6.0f;
      else if (controllers[0].wButtons & XINPUT_GAMEPAD_DPAD_RIGHT) bucket.x += 6.0f; 
   } 
